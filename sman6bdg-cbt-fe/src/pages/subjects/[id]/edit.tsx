...
import SubjectForm from '@/components/Partials/Subjects/SubjectForm'
import type { GetStaticPaths, GetStaticProps } from 'next'
import { serverSideTranslations } from 'next-i18next/serverSideTranslations'
import { useTranslation } from 'next-i18next'

const SubjectsEdit = (): JSX.Element => {
      ...
      <AppLayout
        header={
          <h2 className="text-xl font-semibold leading-tight text-gray-800 ">
            {commonTrans('edit')} {subjectTrans('name')}
          </h2>
        }
        title={`${commonTrans('edit')} ${subjectTrans('name')}`}
      >
        <div className="pt-12">
        ...
        <div className="py-6">
          <div className="mx-auto max-w-7xl space-y-6 sm:px-6 lg:px-8">
            <div className="bg-white p-4 shadow sm:rounded-lg sm:p-8">
              <div>
                <SubjectForm id={router.query.id as string} />
              </div>
            </div>
          </div>
        </div>
